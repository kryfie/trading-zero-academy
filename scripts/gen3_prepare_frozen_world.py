...
for sym in m["symbols"]:
    ...
    if m1_path.exists():
        try:
            ...
            if not cached.empty and int(cached["ts"].min()) <= start_ts and int(cached["ts"].max()) >= end_ts:
                source = cached
                source_mode = "restored_frozen_cache"
        except Exception:
            source = None
    if source is None:
    ...
    if frozen.empty or int(frozen["ts"].min()) != start_ts or int(frozen["ts"].max()) != end_ts:
        raise SystemExit(
            f"{sym}: cannot recreate exact frozen M1 world. expected {start_ts}..{end_ts}, got "
...
print("GENERATION 3 FROZEN WORLD VERIFIED AGAINST GENERATION 1/2 REFERENCE")
